...
if use_imgur and not imgur_available:
  print("Warning: imgurpython module not found. Disabling Imgur upload.")
  use_imgur = False
...
def get_imgur_client(client_id, client_secret):
  try:
    ...
    imgur_pin = input('Please visit %s and enter the pin given to you there: ' % authorization_url)
...
def photobooth_sequence(camera):
  ...
  if animated_strip:
    animated_strip[0].save(
        'photobooth_gif_' + timestamp + '.gif',
        save_all=True,
        append_images=animated_strip[1:],
        duration=100,
        loop=0
    )
...
while True:
  text = input('Press ENTER to start photobooth sequence, or type "peace" to quit: ')
  if (text == 'peace'):
    break
